...
Placeholder.prototype.handleParse = function handleParse(input, lang, context, data, done) {
  var this_ = this;
  var oldResult = util.clone(input.result);
  var phrases = this.grammar.getPhrasesNamed(this.type);
  var currentOptions = util.clone(this.options);

  currentOptions.$call = function(func) {
    var args = 2 <= arguments.length ? Array.prototype.slice.call(arguments, 1) : [];

    var callContext = context || this_.options;
  ...
  function eachPhrase(phrase, done) {
    ...
    function phraseData(option) {
      ...
      phrase.getValue(currentOptions, option.result, handleValue);
    }
    ...
    if (input.stack.slice(0, -1).indexOf(this_) !== -1) {
      done();
      return;
    }


    phrase.parse(input, lang, currentOptions, phraseData, phraseDone);
  }
