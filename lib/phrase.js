...
Phrase.prototype.parse = function(options, data, done) {
  ...
  function doHandleParse() {
    var dataNumber, phraseParseId;

    //bound to preParseOptions
    function applyLimit(input) {
      this.addLimit(phraseParseId, this_.props.limit);
      input = input.addLimit(phraseParseId, dataNumber);
      dataNumber++;
      return input;
    }
    ...
    if (this_.props.limit) {
      ...
      preParseOptions.applyLimit = applyLimit.bind(preParseOptions);
    }
