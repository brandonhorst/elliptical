...
Phrase.prototype._applyAdditions = function() {
  ...
  _.forEach(this.factory.additions, function (value, name) {
    ...
    this_[name] = value;
    this_[setName] = function (newValue) {
      var arg = {};
      arg[name] = newValue;
      this_.factory._additionsCallback(arg);
    };
  });
...
Phrase.prototype.parse = function(input, options, data, done) {
  ...
  function phraseData(input) {
    ...
    var newResult = this_.getValue ? this_.getValue(input.clearTemps()) : input.clearTemps();
  ...
  function doDescribeParse() {
    ...
    if (this_._additionsVersion !== this_.factory._additionsVersion) {
      this_._applyAdditions();
      this_.elements[lang].cache = null;
    }
