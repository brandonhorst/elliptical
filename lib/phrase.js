...
Phrase.prototype._applyAdditions = function () {
  var self = this
  ...
  _.forEach(this.factory.additions, function (value, name) {
    ...
    self[name] = value
    self[setName] = function (newValue) {
      var arg = {}
      arg[name] = newValue
      self.factory._additionsCallback(arg)
    }
  })
...
Phrase.prototype._checkExtensions = function (extensions) {
  ...
  ;['extenders', 'overriders'].forEach(function (name) {
    var cachedExtensions = Object.keys(self['_' + name])
    ...
    newExtensions.forEach(function (newExtension) {
      self['_' + name][newExtension] = extensions[name][newExtension](self.props)
    })

    removedExtensions.forEach(function (removedExtension) {
      delete self['_' + name][removedExtension]
    })
  })
}
...
Phrase.prototype.parse = function (input, options, data, done) {
  ...
  function phraseData (input) {
    ...
    var newResult = self.getValue ? self.getValue(input.clearTemps()) : input.clearTemps()
    oldResult[self.props.id] = newResult
  ...
  function doHandleParse () {
    ...
    function applyLimit (input) {
      ...
      options.addLimit(phraseParseId, self.props.limit)
      dataNumber++
      return newLimit
    }
    ...
    self._handleParse(newInput, options, applyLimit, sendData, done)
  }

  function doDescribeParse () {
    var lang = getBestElementLang(self.elements, options.langs)

    preParseInputData.result = {}

    // if describe has never been executed, execute it and cache it
    if (self._additionsVersion !== self.factory._additionsVersion) {
      self._applyAdditions()
      self.elements[lang].cache = null
    }
    if (!self.elements[lang].cache) {
      self.elements[lang].cache = self.elements[lang].describe.call(self)
    }

    self.elements[lang].cache.parse(new InputOption(preParseInputData), options, phraseData, phraseDone)
  }
  ...
  if (this._handleParse) {
    ...
    asyncEach(Object.keys(this._extenders), function (extender, done) {
      self._extenders[extender].parse(input, options, sendData, done)
    }, extendersDone)
    ...
    asyncEach(Object.keys(this._overriders), function (overrider, done) {
      self._overriders[overrider].parse(input, options, overriderData, done)
    }, overridersDone)
  }
}
