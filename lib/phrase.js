...
Phrase.prototype.parse = function(options, data, done) {
  var preParseOptions = _.clone(options);
  ...
  function parseElement() {
    ...
    if (this_._handleParse) {
      this_._handleParse(preParseOptions, data, done);
    } else {
  ...
  if (options.input.suggestion.words.length > 0 && _.find(options.input.stack, {name: this.name})) {
    done();
    return;
  }
