...
function getBestElementLang(schemas, langs) {
  var baseLangs = langs.map(function (lang) {
    return lang.split('_')[0];
  });

  return pickFirstMatchingKey(schemas, langs.concat(baseLangs)) || 'default';
}
...
function Phrase(obj, props, constructor) {
  ...
  //set up the schemas, or handleParser if it is provided
  if (obj._handleParse) {
    this._handleParse = obj._handleParse.bind(this);
  } else {
    this.elements = {};
    obj.schemas.forEach(function (schema) {
...
Phrase.prototype.parse = function(options, data, done) {
  ...
  if (options.input.suggestion.words.length > 0 && options.input.stack.indexOf(this.name) > -1) {
    done();
    return;
  }

  //add this to the stack before doing anything
  preParseOptions.input = preParseOptions.input.stackPush(this.name);
