  ...
  onCreate: function () {
    ...
    if (this.props.separator) {
      this.actualChildren = this.props.children.slice(0, -1)
        .map(function (child) {
          return addSeparator(child, self.props.separator)
  ...
  _handleParse: function (input, options, applyLimit, data, done) {
    ...
    function parseChild (childIndex, input) {
      function childData (input) {
        var newInputData
        if (childIndex === self.actualChildren.length - 1) {
          newInputData = input.getData()
          newInputData.result[self.props.id] = self.props.value
      ...
      parsesActive++
      self.actualChildren[childIndex].parse(input, options, childData, childDone)
    }
