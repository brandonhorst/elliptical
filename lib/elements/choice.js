var asyncEach = require('async-each');
var asyncEachSeries = require('async-each-series');

var createPhrase = require('../create-phrase');

//Choice::handleParse
module.exports = createPhrase({
  name: 'choice',
  getDefaultProps: function () {
    return {limit: 0};
  },
  handleUnlimited: function(options, data, done) {
    var this_ = this;
    //function eachChild
    // parse the child and call data if it calls its data
    function eachChild(child, done) {

      //function childData
      // set this Choice's value to the value of the child, then call data
      function childData(option) {
        var newResult;
        newResult = option.handleValue(this_.props.id, option.result[child.props.id]);
        data(newResult);
      }

      //parse the child
      child.parse(options, childData, done);
    }

    //Parse each child asyncronously
    asyncEach(this.props.children, eachChild, done);

  },
  handleLimited: function (options, data, done) {
    var this_ = this;
    var childHits = {};

    //function eachChild
    // parse the child and call data if it calls data
    // track hitCount so that childDone can stop it at the limit
    function eachChild(child, done) {
      //function childData
      // set this Choice's value to the value of the child, then call data
      // note that this child has had a hit, so increment the hitcount
      var childData = function(option) {
        var newResult;

        childHits[child.props.id] = true;
        newResult = option.handleValue(this_.props.id, option.result[child.props.id]);
        data(newResult);
      };

      //function childDone
      // if hitCount has hit the limit, then pass a true to to the done() cb.
      //  this will make asyncEachSeries stop executing, but childrenDone
      //  will notice that it is not actually an error and behave accordingly
      function childDone(err) {
        if (err) {
          done(err);
        } else {
          done(Object.keys(childHits).length >= this_.props.limit ? true : null);
        }
      }

      //parse the child, calling childData on data and childDone on done
      child.parse(options, childData, childDone);
    }

    //function childrenDone
    // if err is defined, it could be either an actual error, or just a true
    //  passed from childDone. If it is an error, pass it on. Otherwise,
    //  it was just a termination indicator - call done without an error.
    function childrenDone(err) {
      if (!err || err === true) {
        return done();
      } else {
        return done(err);
      }
    }

    //parse the children syncronously, so we can stop once we hit the limit
    asyncEachSeries(this.props.children, eachChild, childrenDone);
  },
  _handleParse: function (options, data, done) {

    if (this.props.limit === 0) {
      this.handleUnlimited(options, data, done);
    } else {
      this.handleLimited(options, data, done);
    }
  }
});
